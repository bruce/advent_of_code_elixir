defmodule AdventOfCode.Year2022.Day3 do
  ...
  contain jqHRNqRjqzjGDLGL and rsFMfFZSrLrFZsSL. The only item type that appears
  in both compartments is uppercase L. The third rucksack's compartments contain
  ...
  priority of the item type that appears in both compartments of each rucksack
  is 16 (p), 38 (L), 42 (P), 22 (v), 20 (t), and 19 (s); the sum of these is
  157.
  ...
  def parse(input) do
    ...
    for raw_rucksack <- raw_rucksacks do
      raw_rucksack
      |> String.to_charlist()
      |> Enum.split(String.length(raw_rucksack) |> div(2))
    end
  end

  @priorities Enum.to_list(?a..?z) ++ Enum.to_list(?A..?Z)

  @spec priority(item) :: pos_integer()
