defmodule AdventOfCode.Year2022.Day2 do
  ...
  @type game :: [round]
  @type strategy :: Day2.Strategy.Part1.t() | Day2.Strategy.Part2.t()
  ...
  def hand_losses, do: @hand_losses
  ...
  @spec score(round | game | strategy) :: pos_integer()
  def score({hand, hand}) do
    @hand_scores[hand] + 3
  end

  def score({opponent_hand, own_hand}) do
    if @hand_wins[opponent_hand] == own_hand do
      @hand_scores[own_hand]
    else
      @hand_scores[own_hand] + 6
    end
  end

  def score(game) when is_list(game) do
    game
    |> Enum.map(&score/1)
    |> Enum.sum()
  end
  ...
  def score(strategy) do
    strategy.__struct__.to_game(strategy)
    |> score()
  end
