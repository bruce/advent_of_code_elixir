defmodule AdventOfCode.Year2022.Day5 do
  ...
  def parse_drawing(drawing) do
  end

  def parse_instructions(instructions) do
  end

  @spec move(stacks, pos_integer(), {stack_number, stack_number}) :: stacks
  def move(stacks, count, {from_stack_number, to_stack_number}) do
    Enum.reduce(1..count, stacks, fn _, acc ->
      [top | rest] = Enum.at(acc, from_stack_number - 1)
      target = Enum.at(acc, to_stack_number - 1)

      acc
      |> List.replace_at(from_stack_number - 1, rest)
      |> List.replace_at(to_stack_number - 1, [top | target])
    end)
  end

  def generate_message(stacks) do
    for [top | _] <- stacks, into: [], do: top
  end
end
